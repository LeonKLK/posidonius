...
if __name__ == "__main__":
    ...
    initial_time = 1.6e8*365.25 # time [days] where simulation starts
    ...
    star_rotation_period = 1.*24 # hours
    ...
    star_tides = posidonius.effects.tides.CentralBody(star_tides_model)
    # star_tides = posidonius.effects.tides.DisabledModel()
    #
    ...
    planet_radius_of_gyration = np.sqrt(gyration_radius_2) #5.75e-01
    # planet_mass = 9.8160839680e-07 # Solar masses, this value is already in such unit (3.0e-6 solar masses = 1 earth mass)
    planet_mass = 0.3285168664 * posidonius.constants.M_EARTH # Solar masses, this value is already in such unit (3.0e-6 solar masses = 1 earth mass)
    planet_radius = 4810105.0000000000/posidonius.constants.AU # Assume same average density with Earth's

    # first planet
    #////////// Specify initial position and velocity for a stable orbit
    #////// Keplerian orbital elements, in the `asteroidal' format of Mercury code
    a = 0.0619224236258543                # semi-major axis (in AU)
    e = 0.1;                               # eccentricity
    ...
    planet_obliquity = 00. * posidonius.constants.DEG2RAD; # 1.0e-4 # rad
    orbital_period = posidonius.constants.TWO_PI*(np.sqrt( (a*posidonius.constants.AU)**3. / ( posidonius.constants.G_SI*( star_mass*posidonius.constants.M_SUN + planet_mass*posidonius.constants.M_SUN ) ) ) )/86400.
    planet_rotation_period = orbital_period * (1.03) # days
    planet_angular_frequency = posidonius.constants.TWO_PI/(planet_rotation_period) # days^-1
    print(orbital_period, planet_rotation_period, planet_angular_frequency);
    ...
    planet_long_asc_node = planet_keplerian_orbital_elements[5]
    planet_spin = posidonius.calculate_spin(planet_angular_frequency, planet_inclination, planet_obliquity, planet_long_asc_node)
    # print(f"planet_spin: x={planet_spin.x()}, y={planet_spin.y()}, z={planet_spin.z()}")
    # print(planet_angular_frequency, planet_inclination, planet_obliquity, planet_long_asc_node);
