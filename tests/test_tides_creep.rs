...
mod common;
use crate::common::checks::check_uniform_viscosity_coefficient;
use std::path::Path;
...
fn enabled_tides_case() -> posidonius::WHFast {
    ...
    for particle in &mut particles {
        particle.tides = disabled_tides;
        particle.rotational_flattening = disabled_rotational_flattening;
        check_uniform_viscosity_coefficient(&particle);
    }
    //////////////////////////////////////////////////////////////////////////////////
    let star_tides = disabled_tides;
    let star_rotational_flattening = disabled_rotational_flattening;
    let uniform_viscosity_coefficient =
        1.0e22 * posidonius::constants::AU * posidonius::constants::DAY
            / posidonius::constants::M_SUN; // value in kg s^-1 m^-1 => M_SUN day^-1 AU^-1 for the viscosity
    let planet_tidal_model_params = posidonius::CreepCoplanarParameters {
    ...
    let planet_rotational_flattening = posidonius::RotationalFlattening::new(
        posidonius::RotationalFlatteningEffect::OrbitingBody(
            posidonius::RotationalFlatteningModel::CreepCoplanar(planet_tidal_model_params),
        ),
    );
    let planet_tides = posidonius::Tides::new(posidonius::TidesEffect::OrbitingBody(
        posidonius::TidalModel::CreepCoplanar(planet_tidal_model_params),
    ));
