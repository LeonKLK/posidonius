use crate::common::checks::{check_uniform_viscosity_coefficient, evolution_warnings};
use posidonius;
...
#[allow(dead_code)]
pub fn basic_planetary_configuration_for_stellar_kaula(
    star: &posidonius::Particle,
) -> Vec<posidonius::Particle> {
    ...
    let planet1 = super_earth_like(
        star,
        planet1_mass,
        planet1_evolution,
        planet1_semimajor_axis,
        planet1_eccentricity,
        planet1_obliquity,
    );
...
#[allow(dead_code)]
pub fn basic_planetary_configuration_for_planetary_kaula(
    star: &posidonius::Particle,
) -> Vec<posidonius::Particle> {
...
pub fn basic_configuration(star: &posidonius::Particle) -> Vec<posidonius::Particle> {
    ...
    let planet2 = jupiter_like(
        star,
        planet2_mass,
        planet2_evolution,
        planet2_semimajor_axis,
    ...
    let planet3 = jupiter_like(
        star,
        planet3_mass,
        planet3_evolution,
        planet3_semimajor_axis,
        planet3_eccentricity,
        planet3_obliquity,
    );
    ...
    let planet4 = jupiter_like(
        star,
        planet4_mass,
        planet4_evolution,
        planet4_semimajor_axis,
        planet4_eccentricity,
        planet4_obliquity,
    );
    vec![planet1, planet2, planet3, planet4]
}

#[allow(dead_code)]
pub fn super_earth_like(
    star: &posidonius::Particle,
    planet_mass: f64,
    planet_evolution: posidonius::EvolutionType,
    semimajor_axis: f64,
    eccentricity: f64,
    planet_obliquity: f64,
) -> posidonius::Particle {
    match planet_evolution {
        posidonius::EvolutionType::NonEvolving => {}
        _ => {
            panic!(
                "Evolution type should be non evolving to create a terrestrial/earth-like planet!"
            );
        }
    }
    ...
    let (planet_position, planet_velocity) =
        super::tools::calculate_cartesian_coordinates(gm, q, e, i, p, n, l);

    ////// Initialization of planetary spin
    let orbital_period = posidonius::constants::TWO_PI
        * (((a * posidonius::constants::AU).powi(3))
            / (posidonius::constants::G_SI
                * (star.mass * posidonius::constants::M_SUN
                    + planet_mass * posidonius::constants::M_SUN)))
            .sqrt()
        / 86400.; // in days
    ...
    let planet_keplerian_orbital_elements = posidonius::tools::calculate_keplerian_orbital_elements(
        posidonius::constants::G * star.mass * planet_mass,
        planet_position,
        planet_velocity,
    );
    let planet_inclination = planet_keplerian_orbital_elements.inclination;
    let planet_longitude_ascending_node = 0.;
    let planet_spin = super::tools::calculate_spin(
        planet_angular_frequency,
        planet_inclination,
        planet_obliquity,
    ...
    let planet_wind = posidonius::Wind::new(
        posidonius::WindEffect::Disabled,
        planet_wind_k_factor,
        planet_wind_rotation_saturation,
    );
    let planet_disk = posidonius::Disk::new(posidonius::DiskEffect::Disabled);
    let mut planet = posidonius::Particle::new(
        planet_mass,
        planet_radius,
        planet_radius_of_gyration,
        planet_position,
        planet_velocity,
        planet_spin,
    );
    planet.tides = planet_tides;
    planet.rotational_flattening = planet_rotational_flattening;
    check_uniform_viscosity_coefficient(&planet);
    planet.general_relativity = planet_general_relativity;
    planet.wind = planet_wind;
    planet.disk = planet_disk;
    planet.evolution = planet_evolution;
    evolution_warnings(planet_evolution);
    planet
}

#[allow(dead_code)]
pub fn trappist1_h_like(
    star: &posidonius::Particle,
    planet_mass: f64,
    planet_evolution: posidonius::EvolutionType,
    semimajor_axis: f64,
    ...
    ////// Radius of gyration
    let gyration_radius_2: f64 = 0.339_169_539_122_056_7;
...
pub fn earth_like(
    ...
    let planet_radius_factor: f64 = (0.0592 * planet_percent_rock + 0.0975)
        * planet_mass_factor.log10().powi(2)
        + (0.2337 * planet_percent_rock + 0.4938) * planet_mass_factor.log10()
        + 0.3102 * planet_percent_rock
        + 0.7932;
    ...
    let planet_dissipation_factor: f64 =
        2. * posidonius::constants::K2 * k2pdelta / (3. * planet_radius.powi(5));
    /////////////////////////////////////////////////////////////////////////////////
    // Constant Time Lag
    let planet_tidal_model_params = posidonius::ConstantTimeLagParameters {
    ...
    ///////////////////////////////////////////////////////////////////////////////////
    let planet_rotational_flattening_oblate_spheroid_params =
        posidonius::OblateSpheroidParameters {
            love_number: planet_fluid_love_number,
        };
    let planet_rotational_flattening = posidonius::RotationalFlattening::new(
        posidonius::RotationalFlatteningEffect::OrbitingBody(
            posidonius::RotationalFlatteningModel::OblateSpheroid(
                planet_rotational_flattening_oblate_spheroid_params,
            ),
        ),
    );
    let planet_general_relativity =
...
pub fn jupiter_like(
    ...
    match planet_evolution {
        ...
        _ => {
            panic!(
                "Evolution type should be LeconteChabrier2013 or non evolving to create a gaseous/jupiter-like planet!"
            );
        }
    }
