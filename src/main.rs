...
fn main() {
    ...
    match matches.subcommand() {
        Some(("start", start_matches)) => {
            first_universe_integrator_snapshot_filename = start_matches
                .get_one::<String>("start_case_filename")
                .unwrap();
            universe_integrator_snapshot_filename = start_matches
                .get_one::<String>("snapshot_filename")
                .unwrap();
            universe_history_filename = start_matches
                .get_one::<String>("historic_snapshot_filename")
                .unwrap();
            ...
            execution_time_limit =
                Duration::from_secs(start_matches.get_one::<u64>("limit").copied().unwrap_or(0));
        }
        Some(("resume", resume_matches)) => {
            universe_integrator_snapshot_filename = resume_matches
                .get_one::<String>("resume_case_filename")
                .unwrap();
            first_universe_integrator_snapshot_filename = universe_integrator_snapshot_filename;
            universe_history_filename = resume_matches
                .get_one::<String>("historic_snapshot_filename")
                .unwrap();
            silent_mode = resume_matches.get_flag("silent");
            resume = true;

            new_historic_snapshot_period = resume_matches
                .get_one::<f64>("change_historic_snapshot_period")
                .copied()
                .unwrap_or(-1.);
            new_recovery_snapshot_period = resume_matches
                .get_one::<f64>("change_recovery_snapshot_period")
                .copied()
                .unwrap_or(-1.);
            new_time_limit = resume_matches
                .get_one::<f64>("change_time_limit")
                .copied()
                .unwrap_or(-1.);
            execution_time_limit =
                Duration::from_secs(resume_matches.get_one::<u64>("limit").copied().unwrap_or(0));
        }
        _ => unreachable!(),
    }
    ...
    let first_universe_integrator_snapshot_path =
        Path::new(&first_universe_integrator_snapshot_filename);
        ...
        match posidonius::output::restore_snapshot(first_universe_integrator_snapshot_path) {
            ...
            Err(_) => {
                if resume {
                    ...
                    panic!(
                        "[PANIC {} UTC] It was not possible to start the simulation",
                        OffsetDateTime::now_utc()
                            .format(
                                &format_description::parse(
                                    "[year].[month].[day] [hour]:[minute]:[second]"
    ...
    boxed_universe_integrator
        .set_snapshot_periods(new_historic_snapshot_period, new_recovery_snapshot_period);
    ...
    if !resume && universe_integrator_snapshot_path.exists() {
        panic!(
            ...
            OffsetDateTime::now_utc()
                ...
                .unwrap(),
            universe_integrator_snapshot_filename
        );
    ...
    let enabled_execution_time_limit = !matches!(execution_time_limit.as_secs(), 0);
    loop {
            ...
            Ok(recovery_snapshot_time_trigger) => {
                if enabled_execution_time_limit {
                    ...
                    if elapsed >= execution_time_limit {
                        ...
                        boxed_universe_integrator.write_recovery_snapshot(
                            universe_integrator_snapshot_path,
                            &mut universe_history_writer,
                        );
                        println!(
                            "[WARNING {} UTC] Reached execution time limit before simulation completion",
                            OffsetDateTime::now_utc()
                                .format(
                                    &format_description::parse(
                                        "[year].[month].[day] [hour]:[minute]:[second]"
            ...
            Err(e) => {
                println!(
                    ...
                    OffsetDateTime::now_utc()
                        ...
                        .unwrap(),
                    e,
                    first_universe_integrator_snapshot_filename
                );
                break;
            }
        }
    ...
    if resume {
        println!(
            "[INFO {} UTC] Execution time since last resume: {} seconds",
            OffsetDateTime::now_utc()
                .format(
                    &format_description::parse("[year].[month].[day] [hour]:[minute]:[second]")
                        .unwrap()
        ...
        println!(
            "[INFO {} UTC] Execution time: {} seconds",
            OffsetDateTime::now_utc()
                .format(
                    &format_description::parse("[year].[month].[day] [hour]:[minute]:[second]")
                        .unwrap()
